...
int main(){
    ...
    sem_init(&first,0,1);
    sem_init(&second,0,0);
    char command[2][100]={"update table_ws set x = x+1,y=y+1 where id = 1002;",
                          "select * from table_ws where id=1002;"};

    int i;
    int ret=-1;
    pthread_t npid[line_num];
    char local_infile[] = "set global local_infile=1;";
    char load_xml[] = "LOAD XML LOCAL INFILE '/home/database_ws/mysql.xml' INTO TABLE table_ws;";
    char truncate_table[] = "TRUNCATE table_ws;";
    mysql_thread_init();
